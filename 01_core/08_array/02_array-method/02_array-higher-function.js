...
/*
    ...
    따라서 다른 정렬 기준을 사용하기 위해서는 매개변수 compare 이라는
    함수를 전달해야 한다.
*/
numbers.sort();
...
const sum = numbers.reduce((pre, currV) => {
    return pre + currV; // return 한 결과는 다음 콜뱃의 첫번째 인자로 전달    
});
// 하부르타 시간에 분석^^
console.log(`sum : ${sum}`);
...
let result1 = [1, 4, 5, 8, 9].some(item => item % 2 == 0);
...
result = student.find(item => item.name === '김남규')
console.log(result);
